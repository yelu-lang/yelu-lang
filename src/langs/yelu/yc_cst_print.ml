...
(* Print a command's argument list. Labeled-only (Step 2): the supported surface
   is already labeled in the CST (Kw / Kw_flag / Kw_list + positional values), so
   the formatter is a straight pass-through via [pr_arg]. It does NOT rewrite a
   positional cmake keyword into a label — a file written in the positional
   cmake-keyword form is rejected at compile, not silently "fixed" here. (The
   deferred commands that still accept positional keywords — export,
   configure_package_config_file, set_source_files_properties — print them back
   verbatim through the same generic walk.) *)
let pr_cmd_args b _name (args : Cst.arg list) =
