(** fmt matrix smoke — same bridge chain as test_fmt_bridge_smoke,
    ...
    Per cell (option × value):
      ...
      Per-cell directory layout (under _out/fmt/matrix/<opt>_<val>/):
        ...
        ycn-cmake/            RESERVED — future: real cmake on yelu-emitted
                              cmake source (derived via to_normal / from_normal
                              + Yelu_cmake_emit, then re-emitted as cmake text)
        ycn-native/           RESERVED — future: ycn's own backend output
                              (ninja / make / direct invocation; bypasses cmake)
...
let run_cell option value ~project ~reserved : cell_result =
  ...
  (* ycn-cmake/ and ycn-native/ slots are reserved by convention; not
     created until the corresponding backends are wired up. *)
