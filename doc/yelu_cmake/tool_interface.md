...
Each language node has a driver module in [`src/langs/drivers/`](../../src/langs/drivers/)
that exports the operations listed above as OCaml functions. Every operation is either
a direct implementation, a stub (`failwith "not implemented"`), or a composition
through another driver. The uniform structure makes the implementation status
machine-visible:

| Driver | File | Status |
|---|---|---|
| `Yc_driver` | [`yc_driver.ml`](../../src/langs/drivers/yc_driver.ml) | parse (×4), print/compile (×3), eval, convert, check (×2 stubs) |
| `Ycn_driver` | [`ycn_driver.ml`](../../src/langs/drivers/ycn_driver.ml) | parse, print/compile, eval, check |
| `Cmake_ast_driver` | [`cmake_ast_driver.ml`](../../src/langs/drivers/cmake_ast_driver.ml) | parse (JSON CST → stmt list), print, from/to yc |
| `Cmake_text_driver` | [`cmake_text_driver.ml`](../../src/langs/drivers/cmake_text_driver.ml) | parse (tool:parse.py), print (tool:gersemi), eval (tool:cmake), check (tool:cmake) |

Operations marked `failwith "not implemented"` in the driver are visible gaps — they
document where the next implementation work should go (yc→.ye printer, ycn→.ycn printer,
.ycn parser, re-implemented wellform).
