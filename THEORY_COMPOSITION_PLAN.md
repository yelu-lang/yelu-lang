...
| Phase | What | Detail |
| --- | --- | --- |
| **R1** ✓ | Step file backfill | v1 math 6/6, v2 root 11/11, CMakeOnly 13/13 (excluding 2 debug binaries that are parser/lexer drivers, not bridgeable). **49/49 bridgeable step programs (100 %).** |
| **R2** ✓ for R1+R4-a | Bridge fail-case attrition | All R1-demanded constructors landed first batch (May 10). R4-a survey then surfaced ~180 mechanical attrition items across path/file/string/list/cmake_op/property/try fragments + misc flag relaxations. R4-a closed all: path subcommands (17), file subcommands (10), string subcommands (17+), list subcommands (13), cmake_op subcommands (9), property subcommands (6), try_compile extras + try_run, BEFORE/SYSTEM flags relaxed, set(PARENT_SCOPE) relaxed. **Compile-tests bridge survey: 181/194 programs pass** (the 13 remaining are exactly R4-b's semantic batch — see below). Each new construct paired with surface + theory passthrough + lift/lower + emit. |
...
| **R4** ✓ for mechanical | Glue `test_yelu_compile` (194 tests) | Inline bridge assertion added to `check`/`check_vbox`: every program flows through bridge → emit. After R4-a's attrition, 181/194 pass. The 13 remaining cases (block / while / break / continue / return / foreach_range / separate_arguments) are skipped via `bridge_skip` list and gated on R4-b's semantic design conversation. **R4 mechanical pass closed; semantic split surfaced.** |
