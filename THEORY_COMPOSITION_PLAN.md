...
| Phase | What | Detail |
| --- | --- | --- |
| **R1** | Step file backfill (32 files) | v1 math: step4_math, step6_math, step10_math, step11_math, step12_math, step12_multi (6) ✓. v2 root: step1–step11 (11) ✓. CMakeOnly top-level: 5/15 done — project_include, project_include_before, target_scope, target_scope_sib, fetch_content. 8 remain (target_scope_sub, target_scope_sub_sub, link_interface_loop, major_version_selection, find_path, find_library, select_library_configurations, all_find_modules); 2 files in this dir are debug binaries (debug_genex, debug_kwarg), not yelu programs. R1c remaining files are gated on R2 constructor work (see below). |
| **R2** | Bridge fail-case attrition | 13 `_ -> fail "unsupported yelu_cmake …"` catch-alls. Demand-driven from R1 / R4 / R5 / R6. Each new constructor: bridge case + surface mirror + theory mirror + lift/lower + emit. R1b surfaced: `Ytgt_add_library_alias`, `Ytgt_add_executable_alias`, `Ytgt_add_dependencies`, `Ytgt_sources_fs` (with FILE_SET shape), `Ytgt_precompile_headers` — all added (target fragment). R1c remaining requires: `Ytgt_add_library_imported` + `Yprop_set` (link_interface_loop); `Yfind_package` with version/quiet/components (major_version_selection, all_find_modules); `Yexpr_ver_*` exprs + `Ycmake_math` (major_version_selection); `Yc_macro` + `Yvar_unset_cache` + `Yfile_relative_path` (find_path / find_library / select_library_configurations). |
