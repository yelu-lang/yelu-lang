...
| **R2** ✓ for R1 | Bridge fail-case attrition | All R1-demanded constructors landed (May 10): target alias / executable_alias / dependencies / sources_fs (FILE_SET) / precompile_headers / add_library_imported; property set + set_global; find_package (relaxed); version exprs; math; macro; unset_cache; file_relative_path; file_glob; string regex_replace; path get_filename_component; **foreach (control flow)**. Each paired with surface + theory mirror + lift/lower + emit. **R2 still has fail-cases for constructs no R1 file exercises** — these are demand-driven from R4/R5/R6 when they land. |
